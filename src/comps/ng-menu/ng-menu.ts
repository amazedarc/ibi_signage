  ...
  template: `
    <ng-container *ngIf="fileMenuMode">
      <div class="row">
        <section
          id="appNavigatorWasp"
          class="appMenu fill hidden-xs hidden-sm hidden-md col-lg-1"
        >
          <li
            *ngFor="let item of items"
            (click)="listenMenuSelected(item, $event)"
            data-ripple-color="white"
            class="btn btn-default list-group-item navicons"
          >
            <i
              *ngIf="!m_hidden"
              (click)="listenMenuSelected(item, $event)"
              class="iconSize fa {{ item.getFontAwesome }}"
            ></i>
            <span (click)="listenMenuSelected(item, $event)">{{
              item.getTitle
            }}</span>
          </li>
        </section>
              ...
              <ul class="dropdown-menu">
                ...
                <li
                  *ngFor="let item of items"
                  (click)="listenMenuSelected(item, $event)"
                >
                  <a href="#" (click)="listenMenuSelected(item, $event)">{{
                    item.getTitle
                  }}</a>
                </li>
                <li class="divider"></li>
...
export class NgMenu extends Compbaser {
  ...
  private listenWinResize() {
    this.commBroker.onEvent(Consts.Events().WIN_SIZED).subscribe(
      (e: IMessage) => {
